...
const PATTERNS: { re: RegExp; reason: string }[] = [
  { re: /ignore (all |the )?(previous|prior|above) (instructions|prompts)/i, reason: "Prompt-injection attempt: tries to override the agent's instructions." },
  { re: /disregard (your|the) (rules|instructions|guardrails)/i, reason: "Prompt-injection attempt: asks the agent to disregard its rules." },
  ...
  { re: /(transfer|send|wire|pay out|refund)\s+(all|every|\$?\d{3,})/i, reason: "Financial-manipulation attempt: requests a large or blanket payout." },
